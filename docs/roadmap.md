...
5. **Docker Operations probe (closed by `scratch/22-e2-docker-operations-probe.ts`).** Docker-local is available on this machine and can meet the first isolation bar: non-root user, `--network none`, read-only rootfs, tmpfs `/workspace`, no Docker socket, dropped capabilities, `no-new-privileges`, PID limit, memory limit, and cleanup. `docker exec` streams stdout, timeout/abort leave no `sleep` process behind in the probe, and exec-per-op file operations work entirely inside `/workspace`.
6. **File-ops boundary decision.** E.2.1 starts with exec-per-op file Operations inside the container. Host bind mounts are rejected for provider internals because they put file contents back on the host. A richer fs bridge is deferred until exec-per-op proves too slow or insufficient.
...
- Docker-local starts with exec-per-op file Operations inside the container. Defer a persistent fs bridge until we have evidence that exec-per-op is too slow or too limited.
