...
- **Provider selection must fail closed.** If a run/session does not specify a sandbox provider, the control plane must not silently choose host passthrough. The safe outcomes are an explicit "no builtin execution provider configured" error, or a later safe default only after that provider's wiring gates are closed. Host passthrough always requires an explicit unsafe opt-in. Docker-local is factory-only after E.2 and must not become default-reachable until the Docker bash infrastructure-error follow-up is closed ([#22](https://github.com/oneryalcin/open-managed-agents/issues/22)).
