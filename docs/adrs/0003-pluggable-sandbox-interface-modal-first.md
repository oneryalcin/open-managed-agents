...
- **Grep is not sandbox-safe through Operations in Pi 0.75.4.** `createGrepTool` accepts `GrepOperations`, but its implementation still runs host `rg` and only uses Operations for directory checks/context reads. Do not enable grep as a sandbox-backed builtin until we own or upstream a fully delegated grep implementation.
