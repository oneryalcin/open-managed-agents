...
- **`createAgentSessionServices({ cwd })`** gives a clean hook for cwd-bound services — that's where we'll inject our `Sandbox` interface (see [ADR 0003](0003-pluggable-sandbox-interface-modal-first.md)).
