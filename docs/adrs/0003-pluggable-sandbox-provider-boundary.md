...
- **Provider selection must fail closed.** If a run/session does not specify a sandbox provider, the control plane must not silently choose host passthrough. Provider selection is trusted execution configuration supplied by the control plane, operator config, or an authenticated API boundary allowed to choose execution backends; prompt/model content and untrusted agent-definition fields must not influence it. Host passthrough requires two gates: a deployment-level allowance plus the per-session unsafe opt-in. Docker-local is factory-only after E.2 and must hard-reject until the Docker bash infrastructure-error follow-up is closed ([#22](https://github.com/oneryalcin/open-managed-agents/issues/22)).
...
Provider and resource vocabulary to carry forward from the OpenAI sandbox-agents guide and Modal sandbox docs, but not implement in Cycle E.3:
...
Gemini Managed Agents (`https://ai.google.dev/gemini-api/docs/agent-environment`, guide `https://www.philschmid.de/gemini-managed-agents-developer-guide`) is a hosted managed-environment product — ahead of our scope, useful for future resource/lifecycle design, not the current Docker-local/provider-selection path:

- **Egress-proxy secret injection is the sharp version of our env allowlist.** Gemini keeps credentials outside the sandbox and injects them into outbound requests at an egress proxy, so they never exist as files or env vars inside the sandbox. Because in-sandbox code is adversarial (Cycle E.2/#22 proved it can read `/proc` and forge markers), any secret in an allowed env var is readable by the sandboxed code. The target design for secrets is therefore "keep the secret out, inject at egress," which shrinks the env allowlist's job to config-only. Fold this into the eventual secrets/network resource-grant.
- **Take the network allowlist mechanism, keep our default-deny posture.** Gemini defaults to unrestricted outbound with an optional allowlist; Docker-local is `--network none`. When OMA adds a network resource-grant, adopt the allowlist model but anchored on default-deny + explicit egress allowlist — same mechanism, opposite default.
- **Durable, resumable workspace as a separate resource (future fork).** Gemini's `environment_id` (resumable, idle-snapshot, tar download) is separate from the conversation `interaction.id`. OMA already separates conversation/event state from the sandbox provider, but our sandbox is session-lifetime-coupled and ephemeral (disposed on idle-TTL/eviction). Three independent sources now converge on workspace-state-as-a-durable-resource — Gemini's `environment_id`, OpenAI's manifest/serialized-state/snapshot split, and ActiveGraph's fork/replay over a durable log — so "durable resumable sandbox vs ephemeral" is a deliberate future decision, not a now.
