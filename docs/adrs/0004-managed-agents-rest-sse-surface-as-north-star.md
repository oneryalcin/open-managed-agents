...
- **Accept `anthropic-beta: managed-agents-2026-04-01` header.** Anthropic SDKs send it on every call. Our server must accept the header without 400-ing on unrecognized headers; for MVP it's a no-op (we don't gate behavior on it), but failing to accept it breaks every SDK client.
