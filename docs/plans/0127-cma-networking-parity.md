...
rewrite of persisted public data.
...
2. Hosted evidence: probes 61 and 62 establish concrete behavior for empty
   lists, one-level and nested wildcards, HTTPS-only access, package-manager
   access, MCP gating, and invalid hosts.
...
- Each accepted host grants port 443 only. Probe 62 denied HTTP even for an
  otherwise allowed exact or wildcard host. CMA's public contract forbids
  ports in `allowed_hosts`; URL and port-bearing entries are rejected before a
...
1. **Add a hosted-network parser/normalizer** in
   ...
   - Detect hosted-vs-native networking without weakening native strict
     parsing.
   - Validate flags and host syntax, normalize case, expand 443, and expose
     a single wildcard-aware host matcher.
...
2. **Route both environment creation and session admission through it.**
   - `src/control-plane/environments/service.ts`: validate the hosted shape
     before inserting the row; preserve the original JSON on success.
   - `src/control-plane/sessions/service.ts`: replace the current
     `hasEgressNetworkingConfig`-only gate with the shared resolver; reject
     unsupported legacy rows before `prepareFileResources` or
     `prepareSkillResources`.
   - `src/control-plane/wiring.ts`: resolve the normalized hosted policy when
     constructing a per-session egress bundle; return `undefined` for an empty
     allowlist.
...
4. **Add focused contract coverage.**
   - `src/control-plane/egress/__tests__/policy.test.ts`: exact and wildcard
     ...
     normalization, 443-only expansion with HTTP denial, empty allowlist,
     invalid host forms, mixed-shape rejection, duplicate normalization, and
     unsupported flags.
   - Environment API tests: accepted limited response preserves input;
     unrestricted/true flags/invalid hosts return 400 and create no row.
   - Session/wiring tests: hosted limited produces a bundle, empty limited stays
     dark, legacy unsupported rows fail before runtime preparation, and native
     OMA configs retain their existing behavior.
...
- **HTTP/HTTPS mismatch** — probe 62 is HTTPS-only; do not widen hosted inputs
  to HTTP until new evidence and an explicit policy decision justify it.
- **Flag widening** — reject true package/MCP flags rather than inventing a
  partial registry or MCP policy.
- **Legacy rows** — validate at both environment creation and session
  admission, before any sandbox side effect.
