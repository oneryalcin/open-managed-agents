...
| Operational observability | **Shipped (plan 0121, 2026-07-06):** `/health`, fail-closed `/metrics` covering the required telemetry (runtime turns, pending waits, sandbox lifecycle, provider errors, reaper activity, plus HTTP/admission), structured redacted logs. Alert *tooling*/SLOs stay operator-side (suggested-alerts table in dev-deployment). | Gate satisfied for single-node tiers; multi-worker still needs multi-process metric aggregation. |
