#!/usr/bin/env python3
"""Live probe: archive a hosted session paused on a custom-tool requires_action.

Question this answers:
  Does Anthropic's hosted `sessions.archive()` treat a custom-tool
  `requires_action` pause as idle/archivable, or reject it like a running turn?
    ...
    try:
        if env_id is None:
            try:
                ...
                env = client.beta.environments.create(
                    name=f"oma-archive-requires-action-probe-{RUN_ID}",
        ...
        agent = client.beta.agents.create(
            ...
            system=(
                ...
                "When asked, call it exactly once and then wait for its result. "
            ...
            tools=[
                {
                    "type": "custom",
                    "name": TOOL_NAME,
                    "description": "Ask the API caller for archive probe input.",
                    "input_schema": {
                        "type": "object",
        ...
        session = client.beta.sessions.create(
            ...
            title=f"oma-archive-requires-action-probe-{RUN_ID}",
        )
        session_id = session.id
        emit("session.created", session)

        send_response: Any = None
        requires_action_event: Any = None
        ...
        with client.beta.sessions.events.stream(session_id) as stream:
                        ...
                        "content": [
                            {
                                ...
                                "text": (
                                    ...
                                    "question='archive requires_action probe'. "
            ...
            for event in stream:
                ...
                if is_requires_action(event):
                    requires_action_event = event
                    break
                if time.monotonic() > deadline:
                    raise TimeoutError("timed out waiting for requires_action")

        emit("stream.observed_until_requires_action", observed_events)
        if requires_action_event is None:
            raise RuntimeError("missing requires_action before archive attempt")
        ...
        emit("sessions.archive.requires_action", archive_result)
        emit(
        ...
        verdict = {
            "requires_action_observed": requires_action_event is not None,
