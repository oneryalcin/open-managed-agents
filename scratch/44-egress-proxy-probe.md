...
## Result: 8/8 enforcement checks, deterministic
...
| (g) verify-before-inject: wrong upstream CA → 502, secret never leaves | PASS | distinct bogus CA; `http_code=502` (positive marker) + echo request count unchanged; (a) is the correct-CA counterpart |

NOTES (not counted checks): **(d3)** the reflective echo returned the injected
secret in its response body — inherent to boundary injection, mitigated by
path/method-scoped inject grants + optional response redaction. **(private-IP)**
srt has no post-resolution private-IP deny — the allowlisted loopback target was
served with no objection; OMA must add connect-to-pinned-IP in the dial path.
...
- **The reflective-upstream caveat is real** (d3, a NOTE). Because srt pipes
  upstream responses back unmodified, an upstream that echoes the injected
  header returns the real secret into the sandbox. This is inherent to boundary
  injection. Reflectivity is **per-endpoint, not per-host**, so the ADR
  requires (a) **path/method-scoped inject grants** (not host-level trust), and
  (b) a decision on OMA response redaction (Osaurus prior art) — see ADR 0016
  §6.
- **srt has no SSRF/private-IP defense** (NOTE). The proxy happily served a
  loopback target because it was allowlisted; there is no post-DNS-resolution
  IP check. OMA must add one as **connect-to-a-pinned-vetted-IP in the vendored
  dial path** (hostname kept only as SNI) — a filter-only check re-opens DNS
  rebinding; see ADR 0016 §5.
