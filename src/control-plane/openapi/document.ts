...
export const OPENAPI_ROUTE_CONTRACTS: readonly OpenApiRouteContract[] = [
  ...
  route({ method: "get", path: "/v1/environments", operationId: "listEnvironments", tag: "Environments", summary: "List environments", auth: "workspace", beta: "managed", parameters: [limit, page, includeArchived], success: { schema: ref("ForwardEnvironmentPage") } }),
  route({ method: "get", path: "/v1/environments/networking-presets", operationId: "listEnvironmentNetworkingPresets", tag: "Environments", summary: "List safe networking presets and deployment capability", auth: "workspace", beta: "managed", success: { schema: ref("EnvironmentNetworkingPresetCatalog") } }),
  route({ method: "post", path: "/v1/environments/networking-presets/validate", operationId: "validateEnvironmentNetworkingHosts", tag: "Environments", summary: "Validate and normalize a custom hostname allowlist", auth: "workspace", beta: "managed", requestBody: jsonBody(ref("ValidateEnvironmentNetworkingHostsRequest")), success: { schema: ref("ValidateEnvironmentNetworkingHostsResponse") } }),
  ...
  route({ method: "post", path: "/v1/environments/{id}/archive", operationId: "archiveEnvironment", tag: "Environments", summary: "Archive an environment", auth: "workspace", beta: "managed", parameters: [pathParam("id", "Environment ID.")], success: { schema: ref("Environment") } }),
  route({ method: "delete", path: "/v1/environments/{id}", operationId: "deleteEnvironment", tag: "Environments", summary: "Delete an unreferenced environment", description: "OMA deliberately refuses deletion while any durable session references the environment.", auth: "workspace", beta: "managed", parameters: [pathParam("id", "Environment ID.")], success: { schema: ref("DeletedEnvironment") } }),
...
const schemas: Record<string, JsonSchema> = {
  ...
  ModelInput: { oneOf: [{ type: "string" }, { type: "object", required: ["id"], properties: { provider: { type: "string", minLength: 1 }, id: { type: "string", minLength: 1 }, speed: { type: "string", enum: ["standard", "fast"] } }, additionalProperties: false }] },
  ...
  Agent: { type: "object", required: ["id", "type", "name", "model", "version", "created_at", "updated_at", "archived_at"], properties: { id: { type: "string" }, type: { const: "agent" }, name: { type: "string" }, model: { type: "object", required: ["provider", "id", "speed"], properties: { provider: { type: "string" }, id: { type: "string" }, speed: { enum: ["standard", "fast"] } }, additionalProperties: false }, system: ref("NullableString"), description: ref("NullableString"), tools: { type: "array", items: ref("JsonObject") }, skills: { type: "array", items: ref("JsonObject") }, mcp_servers: { type: "array", items: ref("JsonObject") }, metadata: ref("StringMap"), multiagent: { type: ["object", "null"] }, version: { type: "integer", minimum: 1 }, created_at: { type: "string", format: "date-time" }, updated_at: { type: "string", format: "date-time" }, archived_at: { type: ["string", "null"], format: "date-time" } }, additionalProperties: false },
  CreateEnvironmentRequest: { type: "object", required: ["name", "config"], properties: { name: { type: "string", minLength: 1 }, config: ref("JsonObject"), metadata: ref("StringMap") }, additionalProperties: false },
  Environment: { type: "object", required: ["id", "type", "name", "config", "metadata", "created_at", "updated_at", "archived_at"], properties: { id: { type: "string" }, type: { const: "environment" }, name: { type: "string" }, config: ref("JsonObject"), metadata: ref("StringMap"), created_at: { type: "string", format: "date-time" }, updated_at: { type: "string", format: "date-time" }, archived_at: { type: ["string", "null"], format: "date-time" } } },
  DeletedEnvironment: { type: "object", required: ["type", "id"], properties: { type: { const: "environment_deleted" }, id: { type: "string" } }, additionalProperties: false },
  EnvironmentNetworkingPresetCatalog: { type: "object", required: ["type", "deployment", "presets", "custom"], properties: { type: { const: "environment_networking_presets" }, deployment: { type: "object", required: ["provider", "egress_supported", "reason"], properties: { provider: { type: ["string", "null"] }, egress_supported: { type: "boolean" }, reason: { type: ["string", "null"] } }, additionalProperties: false }, presets: { type: "array", items: { type: "object", required: ["id", "version", "name", "description", "networking"], properties: { id: { type: "string" }, version: { type: "integer", minimum: 1 }, name: { type: "string" }, description: { type: "string" }, networking: { type: "object", required: ["type", "allowed_hosts"], properties: { type: { const: "limited" }, allowed_hosts: { type: "array", items: { type: "string" } } }, additionalProperties: false } }, additionalProperties: false } }, custom: { type: "object", required: ["https_only", "wildcard_matches_bare_domain"], properties: { https_only: { const: true }, wildcard_matches_bare_domain: { const: false } }, additionalProperties: false } }, additionalProperties: false },
  ValidateEnvironmentNetworkingHostsRequest: { type: "object", required: ["allowed_hosts"], properties: { allowed_hosts: { type: "array", items: { type: "string" } } }, additionalProperties: false },
  ValidateEnvironmentNetworkingHostsResponse: { type: "object", required: ["allowed_hosts"], properties: { allowed_hosts: { type: "array", items: { type: "string" } } }, additionalProperties: false },
