        ...
        {files.map((f, i) => (
          <div className={'trow' + (f.dl ? '' : ' inert-row')} key={i}>
            ...
            <span className="td" style={{ width:110 }}>
              {f.dl ? <a className="dl" href="#" onClick={(e) => { e.preventDefault(); OmaConsoleApi.downloadFile(f.href, f.name); }}><Icon name="download" size={14} />Download</a> : <span className="inert">— mounted input</span>}
            </span>
          </div>
        ))}
      </div>
